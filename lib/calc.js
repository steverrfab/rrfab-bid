...
function computePlateRow(row) {
  // If weight_lb is provided (e.g., from external estimator), use it directly
  if (row.weight_lb && +row.weight_lb > 0) {
    const totalWeight = +row.weight_lb;
    const totalPrice = (+row.cost_factor || 0) * totalWeight / 100;
    return { sqft: 0, psf: 0, totalWeight, totalPrice };
  }
...
function compute(est, overrides, shapes, plates, miscRows, aiscLookup, extras) {
  ...
  const hardwareWt = (+est.hardware_weight || 0) > 0 ? +est.hardware_weight : materialWeight;
  ...
  const hardware = hardwareWt * (+est.hardware_rate || 0);
  const galv = (+est.galv_weight || 0) * (+est.galv_rate || 0);
  const sec3ExtrasTotal = extras.filter(x => x.section === 3)
    .reduce((s, x) => s + (+x.qty || 0) * (+x.rate || 0), 0);
  const finishes = paint + consumables + handling + hardware + galv + sec3ExtrasTotal;
  ...
  return {
    ...
    processingLabor,
    fabLabor,
    paint, consumables, handling, hardware, galv, finishes,
