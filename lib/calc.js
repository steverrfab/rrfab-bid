...
// DO NOT "improve" this to strip inch marks or resolve bare fractions.
//
// It looks wrong, and in isolation it is: '3/8"' misses the table and falls
// through to parseFloat, which stops at the slash and reads it as 3 INCHES
// thick. But bids quoted before August 2026 have their plate thicknesses stored
// in exactly that shape, and they were sent to GCs at the numbers this produces.
// Making the lookup smarter silently reprices every one of them.
//
// The import path is where this is fixed: parsePlateDims now stores clean
// thicknesses in the same spelling PLATE_PSF uses ('3/8', '1 1/4'), so anything
// imported from here on hits the table on the first line and never reaches the
// parseFloat fallback at all. Old rows keep computing exactly as they always
// have, untouched.
...
// The plate weight lookup exactly as it behaved BEFORE inch marks were handled,
// when '3/8"' missed the table and parseFloat read it as three inches thick.
//
// This is wrong, deliberately. It exists for one reason: a one-time data fix
// needs to reproduce the weight an already-quoted bid was priced at, so those
// bids can be frozen at the number that actually went out to the GC. Nothing
// else may call it, and nothing new should ever be priced with it.
function legacyPlateUnitWeight(thickness) {
  if (thickness == null) return 0;
  const key = String(thickness).trim();
  if (PLATE_PSF[key] != null) return PLATE_PSF[key];
  const dec = parseFloat(key);
...
function compute(est, overrides, shapes, plates, miscRows, aiscLookup, extras) {
  ...
  // Finishes
  // Paint, consumables and handling are charged against the weight of steel
  // going through them, which is the material weight computed above -- the same
  // number the takeoff sections show. NULL means automatic: the weight follows
  // that total, so Cost Inputs can never drift away from the takeoff again.
  // These used to be stored numbers stamped once at import and never updated,
  // so every edit to the takeoff afterwards widened the gap.
  //
  // A stored number is a deliberate override and is used exactly as entered,
  // including 0. Every bid saved before this existed carries a stored number,
  // so no existing bid reprices by so much as a pound.
  //
  // Galvanizing is NOT in this group. It is only some jobs, so it stays a plain
  // stored weight that defaults to 0 and is entered by hand.
  const finishWeight = (v) => (v == null || v === '' ? materialWeight : +v || 0);
  const paintWt = finishWeight(est.paint_weight);
  const consWt = finishWeight(est.consumables_weight);
  const handlingWt = finishWeight(est.handling_weight);
