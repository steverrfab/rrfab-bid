...
function plateUnitWeight(thickness) {
  ...
  // Strip inch and foot marks first. '3/8"' is a perfectly normal way to write
  // a thickness, but with the mark attached it missed the table and fell
  // through to parseFloat, which read it as 3 INCHES thick -- eight times the
  // real weight. Same for '1/2"' at twenty-four times.
  const key = String(thickness).replace(/["']/g, '').trim();
  if (PLATE_PSF[key] != null) return PLATE_PSF[key];
  // A bare fraction or mixed number that is not one of the stock thicknesses.
  // parseFloat cannot read these: it stops at the slash and turns 3/8 into 3.
  const frac = key.match(/^(\d+)[\s-]+(\d+)\/(\d+)$/) || key.match(/^(\d+)\/(\d+)$/);
  if (frac) {
    const dec = frac.length === 4
      ? +frac[1] + (+frac[2] / +frac[3])
      : +frac[1] / +frac[2];
    return dec > 0 ? dec / 12 * 490 : 0;
  }
