...
//
// There are two possible destinations and the timer, the snapshot and the
// reporting are shared between them. Whichever is configured is the one used;
// configuring neither leaves the module inert and says so once in the log.
//
// SharePoint (preferred here, because the company is already on Microsoft 365
// and the app registration exists for sending mail):
//   BACKUP_SP_SITE       e.g. rrfabrication.sharepoint.com:/sites/Operations
//   BACKUP_SP_FOLDER     optional, default 'BidToolBackups'
//   BACKUP_SP_LIBRARY    optional, a library other than the site's default
...
const sharepoint = require('./offsite_sharepoint');
...
const state = {
  configured: false,
  destination: null,          // 'sharepoint' | 's3' | null
  lastAttemptAt: null,
  lastSuccessAt: null,
  lastKey: null,
  lastBytes: null,
...
// SharePoint wins when both are set up. Not arbitrary: it is the one whose
// credentials are already in use elsewhere in this app, so it is the one more
// likely to still be valid.
function destination() {
  if (sharepoint.isConfigured()) return 'sharepoint';
...
async function runOnce(db) {
  ...
  try {
    ...
    if (dest === 'sharepoint') {
      const out = await sharepoint.upload(body, now);
      key = out.path;
      where = 'SharePoint ' + sharepoint.CFG.site;
    } else {
      key = objectKey(now);
    ...
    console.log('[offsite] backed up ' + body.length + ' bytes to ' + where + '/' + key);
  } catch (err) {
...
function start(db) {
  ...
  state.configured = state.destination !== null;
  if (!state.configured) {
    // A half-finished SharePoint setup is the likely case now, so say which
    // variable is missing rather than listing everything from scratch.
    const spPartial = sharepoint.missingConfig().length < 4;
    console.log('[offsite] off-site backup is NOT configured. ' +
      (spPartial
        ? 'SharePoint is partly set up; still missing: ' + sharepoint.missingConfig().join(', ') + '. '
        : 'Set BACKUP_SP_SITE for SharePoint, or the four BACKUP_S3_* variables for object storage. ') +
      'The database is currently backed up nowhere.');
    return state;
  }
  console.log('[offsite] off-site backup on, every ' + CFG.intervalMin + ' minute(s), to ' +
    (state.destination === 'sharepoint' ? 'SharePoint ' + sharepoint.CFG.site : CFG.bucket));
...
module.exports = {
  start, runOnce, state, isConfigured, objectKey, signedHeaders, CFG,
  destination, s3Configured, sharepoint
};
