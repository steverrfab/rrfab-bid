...
//   AZURE_CLIENT_ID
//   AZURE_CLIENT_SECRET
//   AZURE_SENDER_USER       (the mailbox to send from, e.g. office@rrfabrication.org)
//   AZURE_SENDER_DISPLAY    (optional, defaults to "R&R Bid")
//
// Fallback path: SMTP via nodemailer (blocked on Railway, kept for self-host).
...
async function sendReadyToSubmit(bundle, recipients, pdfBuffer) {
  ...
  if (!isConfigured()) {
    ...
    return { ok: true, sent: 0, skipped: 'email not configured (set AZURE_* vars for Graph, or SMTP_* for SMTP)' };
  }
...
module.exports = { sendReadyToSubmit, isConfigured, graphConfigured, smtpConfigured };
