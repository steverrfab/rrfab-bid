...
async function sendReadyToSubmit(bundle, recipients, pdfBuffer) {
  ...
  try {
    ...
    console.log(`[email] ready-to-submit sent via ${mode}: ${result.sent} recipient(s) -> ${recipients.map(r => r.email).join(', ')}`);
    return result;
  } catch (err) {
...
// ---------- Resubmitted ----------
// Sent when a Submitted bid is resubmitted in place. Carries the required reason,
// who resubmitted it, and the auto-detected list of what changed. No PDF is
// attached (mirrors the resubmit route, which regenerates the PDF on demand).
async function sendResubmitNotification(bundle, recipients, meta = {}) {
  if (!recipients || recipients.length === 0) {
    return { ok: true, sent: 0, skipped: 'no recipients configured' };
  }
  if (!isConfigured()) {
    console.warn('[email] not configured; would have notified (resubmit):', recipients.map(r => r.email).join(', '));
    return { ok: true, sent: 0, skipped: 'email not configured' };
  }
  const e = bundle.estimate || {};
  ...
  const viewUrl = `${frontendUrl()}/#/estimate/${e.id}`;
  const subject = `RESUBMITTED: ${e.project_name || 'Untitled'} — Bid #${e.bid_number || e.id}`;
  const changesHtml = changes.length
    ? `<ul style="margin:6px 0 0;padding-left:20px;font-size:13px;color:#333">${changes.map(c => `<li style="margin:3px 0">${esc(c)}</li>`).join('')}</ul>`
    : `<p style="font-size:13px;color:#888;margin:6px 0 0">No field-level changes were detected (or this is the first resubmit since change tracking was enabled).</p>`;
  const html = `
<div style="font-family:Arial,sans-serif;max-width:600px">
  ...
  <div style="padding:24px 20px;background:#fff">
    <p style="font-size:18px;font-weight:700;color:#ff6b35;margin:0 0 4px">Bid Resubmitted</p>
    <p style="font-size:13px;color:#666;margin:0 0 16px">The proposal date has been set to today and the PDF regenerated. This reuses the same bid number (not a new revision).</p>
    <table style="width:100%;font-size:13px;border-collapse:collapse">
      <tr><td style="color:#888;width:140px;padding:4px 0">Project</td><td style="font-weight:600">${esc(e.project_name) || '—'}</td></tr>
      <tr><td style="color:#888;padding:4px 0">Bid #</td><td>${esc(e.bid_number) || '—'}</td></tr>
      <tr><td style="color:#888;padding:4px 0">Client / GC</td><td>${esc(e.client_gc) || '—'}</td></tr>
      <tr><td style="color:#888;padding:4px 0">Resubmitted by</td><td>${esc(who) || '—'}</td></tr>
      <tr><td style="color:#888;padding:4px 0">Sell Price</td><td style="font-weight:700;font-size:15px">${money(num.sellPrice)}</td></tr>
    </table>
    <p style="margin:18px 0 4px;color:#888;font-size:12px;text-transform:uppercase;letter-spacing:.5px">Reason for change</p>
    <div style="background:#f5f5f5;border-radius:6px;padding:12px 14px;font-size:14px;white-space:pre-wrap;color:#222">${esc(note) || '(none given)'}</div>
    <p style="margin:18px 0 4px;color:#888;font-size:12px;text-transform:uppercase;letter-spacing:.5px">What changed</p>
    ${changesHtml}
  ...
  const changesText = changes.length ? changes.map(c => '  - ' + c).join('\n') : '  (none detected)';
  const text = `RESUBMITTED: ${e.project_name || 'Untitled'} — Bid #${e.bid_number || e.id}\n`
    + `Resubmitted by: ${who || '—'}\n`
    + `Proposal date set to today; PDF regenerated. Same bid number (not a new revision).\n\n`
    + `Reason for change:\n${note || '(none given)'}\n\n`
    + `What changed:\n${changesText}\n\n`
    + `Sell Price: ${money(num.sellPrice)}\nView bid: ${viewUrl}`;
  const mode = graphConfigured() ? 'graph' : 'smtp';
  try {
    if (mode === 'graph') {
      const sender = process.env.AZURE_SENDER_USER;
      ...
      const res = await withTimeout(
        ...
        SEND_TIMEOUT_MS, 'Graph sendMail (resubmit)'
      );
      if (res.status !== 202) { const t = await res.text(); throw new Error(`Graph sendMail failed: ${res.status} ${t}`); }
    } else {
      const transport = buildSmtpTransport();
      await withTimeout(
        transport.sendMail({
        ...
        SEND_TIMEOUT_MS, 'SMTP send (resubmit)'
      );
    }
    console.log(`[email] resubmit sent via ${mode}: ${recipients.length} recipient(s) -> ${recipients.map(r => r.email).join(', ')}`);
    return { ok: true, sent: recipients.length };
  } catch (err) {
    console.error(`[email] resubmit send failed (${mode}):`, err.message || err);
    return { ok: false, sent: 0, error: err.message || String(err) };
  }
}
...
async function sendInvite(toEmail, toName, inviteUrl) {
  ...
  return sendLinkMail(toEmail, toName, subject, html, text, inviteUrl, 'invite');
}

// Password reset. Same link mechanism as an invite (the token lands on the
// set-your-password screen); only the wording differs.
async function sendPasswordReset(toEmail, toName, resetUrl) {
  const subject = 'Reset your R&R Bid password';
  const html = `
    <div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;color:#222;max-width:600px;">
      <p style="margin:0 0 14px 0;">Hi${toName ? ' ' + toName : ''},</p>
      <p style="margin:0 0 14px 0;">An administrator requested a password reset for your <b>R&amp;R Bid</b> account.</p>
      <p style="margin:0 0 20px 0;">Click the button below to choose a new password. This link expires in 48 hours. Your current password keeps working until you use it.</p>
      <a href="${resetUrl}" style="display:inline-block;background:#2563eb;color:#fff;font-weight:600;font-size:14px;padding:10px 24px;border-radius:6px;text-decoration:none;">Choose a New Password</a>
      <p style="margin:24px 0 0 0;font-size:12px;color:#888;">Or paste this link: ${resetUrl}</p>
      <hr style="border:none;border-top:1px solid #eee;margin:24px 0 12px 0;">
      <p style="font-size:11px;color:#999;margin:0;">Sent by R&amp;R Bid. If you weren't expecting this, you can ignore it.</p>
    </div>
  `;
  const text = `Reset your R&R Bid password: ${resetUrl}\n\nThis link expires in 48 hours.`;
  return sendLinkMail(toEmail, toName, subject, html, text, resetUrl, 'password reset');
}

async function sendLinkMail(toEmail, toName, subject, html, text, url, label) {
  if (!isConfigured()) {
    console.log(`[email] ${label} not sent (email not configured). URL: ${url}`);
    return { ok: false, skipped: 'email not configured' };
  }
  ...
  try {
    if (graphConfigured()) {
      ...
      const res = await withTimeout(
        ...
        SEND_TIMEOUT_MS, `Graph sendMail (${label})`
      );
      if (res.status !== 202) {
        const t = await res.text();
        throw new Error(`Graph sendMail failed: ${res.status} ${t}`);
      ...
      await withTimeout(
        ...
        SEND_TIMEOUT_MS, `SMTP send (${label})`
      );
    }
    return { ok: true };
  } catch (err) {
...
// ---- Bid due reminder ----
// One message per estimator per bid per window; lib/bid_reminders.js decides
// when and logs the send. `kind` is '24h' or 'morning_of'. `isOwner` is false
// for an admin who opted into copies of other estimators' reminders.
async function sendBidReminder(toEmail, toName, bid, kind, isOwner = true) {
  const { friendlyDue } = require('./bid_due');
  const when = friendlyDue(bid.bid_date, bid.bid_time);
  const name = bid.project_name || 'Untitled bid';
  const url = frontendUrl() + (bid.link_path || '/#/estimate/' + bid.id);
  const lead = kind === 'morning_of' ? 'Bid due today' : 'Bid due tomorrow';
  const subject = `${lead}: ${name}`;
  const whose = isOwner ? 'Your bid' : `${bid.owner_name || 'An estimator'}'s bid`;
  const html = `
    <div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;color:#222;max-width:600px;">
      <p style="margin:0 0 14px 0;">Hi${toName ? ' ' + toName : ''},</p>
      <p style="margin:0 0 14px 0;">${whose} for <b>${name}</b>${bid.client_gc ? ' (' + bid.client_gc + ')' : ''} is due <b>${when}</b> Eastern.</p>
      <p style="margin:0 0 20px 0;color:#555;">Status: ${bid.status || 'Draft'}${bid.bid_number ? ' &middot; Bid #' + bid.bid_number : ''}</p>
      <a href="${url}" style="display:inline-block;background:#ff6b35;color:#fff;font-weight:600;font-size:14px;padding:10px 24px;border-radius:6px;text-decoration:none;">${bid.id ? 'Open the bid' : 'Open the Bid Calendar'}</a>
      <p style="margin:24px 0 0 0;font-size:12px;color:#888;">Or paste this link: ${url}</p>
      <hr style="border:none;border-top:1px solid #eee;margin:24px 0 12px 0;">
      <p style="font-size:11px;color:#999;margin:0;">Sent by R&amp;R Bid. Change your reminders under Settings &rsaquo; User Settings.</p>
    </div>
  `;
  const text = `${whose} for ${name}${bid.client_gc ? ' (' + bid.client_gc + ')' : ''} is due ${when} Eastern.\n\n${bid.id ? 'Open the bid' : 'Open the Bid Calendar'}: ${url}`;
  return sendLinkMail(toEmail, toName, subject, html, text, url, 'bid reminder');
}

// ---- Won notification ----
async function sendWonNotification(bundle, recipients) {
  if (!recipients || !recipients.length) {
    console.warn('[email] won notification skipped: no active recipients');
    return { ok: true, sent: 0, skipped: 'no recipients configured' };
  }
  if (!isConfigured()) {
    console.warn('[email] won notification skipped (email not configured); would have notified:', recipients.map(r => r.email).join(', '));
    return { ok: true, sent: 0, skipped: 'email not configured' };
  }
  const e = bundle.estimate;
  const num = displayNumbers(bundle);
  ...
  function money2(n) { const v = Math.round(+n || 0); return '$' + v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  const html = `
<div style="font-family:Arial,sans-serif;max-width:600px">
  <div style="background:#1a1a1a;padding:16px 20px">
    <span style="color:#ff6b35;font-weight:700;font-size:16px">R&amp;R FABRICATION</span>
  ...
  try {
    if (mode === 'graph') {
      ...
      // Graph returns 202 Accepted on success. Anything else means the message
      // was rejected and NOBODY got it — without this check the failure was
      // swallowed and reported as ok, with nothing in the logs.
      if (res.status !== 202) {
        const t = await res.text();
        throw new Error(`Graph sendMail failed: ${res.status} ${t}`);
      }
    ...
    console.log(`[email] won sent via ${mode}: ${recipients.length} recipient(s) -> ${toList}`);
    return { ok: true, sent: recipients.length };
  } catch (err) {
    console.error(`[email] won notification failed (${mode}) for ${recipients.length} recipient(s) -> ${toList}:`, err.message || err);
    return { ok: false, sent: 0, error: err.message || String(err) };
  }
}
...
// Operations-only alerts, opt-in through B2_BACKUP_ALERT_TO. No bid contents,
// attachments, signed links or credentials are included in the message.
async function sendBackupNotification(toEmail, { kind, lastSuccessAt, lastError }) {
  if (!isConfigured()) return { ok: false };
  const subject = kind === 'recovery'
    ? 'R&R Bid: Backblaze backup recovered'
    : 'R&R Bid: Backblaze backup needs attention';
  const text = [
    kind === 'recovery'
      ? 'A Backblaze backup has now passed its download and database integrity checks.'
      : 'The Backblaze backup or its restore check failed. The service will retry.',
    'Last verified backup (UTC): ' + (lastSuccessAt || 'none yet'),
    kind === 'failure' ? 'Check: ' + (lastError || 'backup validation failed') : '',
    'Review the rrfab-bid service backup status and Railway logs.',
    'Existing SharePoint backups run separately. This email does not report their status.'
  ].filter(Boolean).join('\n\n');
  try {
    if (graphConfigured()) {
      const sender = process.env.AZURE_SENDER_USER;
      ...
      const response = await fetch(`${GRAPH_BASE}/users/${encodeURIComponent(sender)}/sendMail`, {
        ...
        body: JSON.stringify({
          message: { subject, body: { contentType: 'Text', content: text },
    ...
    const transport = buildSmtpTransport();
    try {
      await transport.sendMail({ from: smtpFromAddress(), to: toEmail, subject, text });
      return { ok: true };
    } finally { transport.close(); }
  } catch {
    // Provider errors can contain sensitive details; status/logs use a fixed error.
    return { ok: false };
  }
}

module.exports = { sendReadyToSubmit, sendResubmitNotification, sendFeedback, sendInvite, sendPasswordReset, sendBidReminder, sendWonNotification, sendAccessRequestNotification, sendBackupNotification, isConfigured, graphConfigured, smtpConfigured };
