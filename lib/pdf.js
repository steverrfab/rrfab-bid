...
function writeProposalToDoc(doc, bundle) {
  ...
  // SCOPE OF WORK (mirrors the Client Proposal: proposal_scope, falling back to scope)
  ...
  let coreItems;
  if (isPO) {
    // Process-only: single lump-sum line (pre-tax = quoted minus tax)
    coreItems = [{ desc: 'Process & fabrication of structural steel per attached scope', amt: (pc.quoted || 0) - (pc.taxAmt || 0) }];
  } else {
    coreItems = [
