...
function writeProposalToDoc(doc, bundle) {
  ...
  let coreItems;
  if (isPO) {
    // Process-only: single lump-sum line (pre-tax = quoted minus tax). Description editable.
    const poDesc = (e.proposal_line_1_desc || '').trim() || 'Process & fabrication of structural steel per attached scope';
    coreItems = [{ desc: poDesc, amt: (pc.quoted || 0) - (pc.taxAmt || 0) }];
  } else {
    coreItems = [
  ...
  doc.font('Helvetica-Bold').fontSize(10).fillColor('#1a1a1a').text(fmt(isPO ? ((pc.quoted || 0) - (pc.taxAmt || 0)) : c.totalBid), amtColX, ry, { width: amtColW - 4, align: 'right', lineBreak: false });
  doc.y = ry + 16;
  ...
  doc.font('Helvetica').fontSize(10).fillColor('#1a1a1a').text(fmt(isPO ? (pc.taxAmt || 0) : (c.tax ? c.tax.amount : 0)), amtColX, ry, { width: amtColW - 4, align: 'right', lineBreak: false });
  doc.y = ry + 16;
  ...
  doc.font('Helvetica-Bold').fontSize(12).fillColor('#1a1a1a').text(fmt(isPO ? (pc.quoted || 0) : c.totalFurnishInstall), amtColX, ry, { width: amtColW - 4, align: 'right', lineBreak: false });
  doc.y = ry + 22;

  // -----------------------------------------------------------------------
  // ADD ALTERNATES (priced separately; summary ADD price plus itemized lines)
  // -----------------------------------------------------------------------
  const alternates = (bundle.alternates || []).filter(ab => ab && ab.estimate);
  if (alternates.length) {
    ...
    alternates.forEach((ab, ai) => {
      ...
      let altItems;
      if (aIsPO) {
        altItems = [{ desc: 'Process & fabrication per scope', amt: (apc.quoted || 0) - (apc.taxAmt || 0) }];
      } else {
        altItems = [
          { desc: 'Structural steel material - furnished', amt: ac.materialPrice * amf },
          { desc: 'Shop fabrication and finishes', amt: ((ac.fabHours || 0) + (ac.paint || 0) + (ac.consumables || 0) + (ac.handling || 0)) * amf },
          { desc: 'Detailing and PE-stamped shop drawings', amt: (((+ae.struct_detailing||0)*(+ae.struct_detailing_qty||1)) + ((+ae.misc_detailing||0)*(+ae.misc_detailing_qty||1)) + ((+ae.pe_stamp||0)*(+ae.pe_stamp_qty||1))) * amf },
          { desc: 'Freight to jobsite', amt: (+ae.freight || 0) * (+ae.freight_qty || 1) * amf },
          { desc: 'Field erection, equipment, and rigging', amt: ((ac.erectionLabor || 0) + (+ae.erection_equip || 0) * (+ae.erection_equip_qty || 1)) * amf },
          { desc: 'Galvanizing', amt: (ac.galv || 0) * amf },
          { desc: 'Processing labor', amt: (ac.processingLabor || 0) * amf }
        ];
