...
const { footToWholeDollars } = require('./round');
...
function fmt(n) {
  // Bids are quoted in whole dollars: rounded to the nearest dollar, printed with cents (e.g. $50.00).
...
function writeProposalToDoc(doc, bundle) {
  ...
  // When a price-to-win override is set, the itemized lines no longer sum to the
  // quoted total, so the client sees one bottom-line amount. Otherwise show the
  // normal itemized breakdown (which does sum to the total).
  const coreItems = view.priceToWin != null
    ? [{ desc: (e.proposal_scope || e.scope || '').trim() ? 'Furnish, fabricate, and install per scope of work above' : 'Furnish, fabricate, and install per attached scope', amt: view.subtotal }]
    : view.lines.filter(l => !l.hidden).map(l => ({ desc: l.desc, amt: l.amount }));
  // Whole-dollar itemized lines that still foot to the printed subtotal.
  if (view.priceToWin == null) footToWholeDollars(coreItems, 'amt', view.subtotal);
    ...
    alternates.forEach((ab, ai) => {
      ...
      if (aIsPO) {
        ...
        altItems = [
          ...
          // Processing labor bundled into Shop Fabrication (line 2); no separate line 7.
          { desc: altLineDesc(2), amt: ((ac.fabHours || 0) + (ac.paint || 0) + (ac.consumables || 0) + (ac.handling || 0) + (ac.processingLabor || 0)) * amf },
        ...
        if ((+ae.sub_joist_deck || 0) > 0) altItems.push({ desc: subLabel(ae.sub_joist_deck_label, 'Joist and Deck - by Subcontractor'), amt: (+ae.sub_joist_deck||0)*(+ae.sub_joist_deck_qty||1)*amf });
        if ((+ae.sub_erection || 0) > 0) altItems.push({ desc: subLabel(ae.sub_erection_label, 'Erection - by Subcontractor'), amt: (+ae.sub_erection||0)*(+ae.sub_erection_qty||1)*amf });
      ...
      // Whole-dollar alternate lines that foot to the pre-tax portion of the ADD
      // price, then append the (already whole-dollar) sales tax line.
      footToWholeDollars(altItems, 'amt', addPrice - aTax);
      altItems = altItems.filter(it => it.amt !== 0);
      if (Math.round(aTax) !== 0) altItems.push({ desc: 'Sales tax', amt: Math.round(aTax) });
  ...
  // Site Specific — only printed when the user actually entered rows. A blank
  // list prints nothing (the old "None noted." placeholder is gone), so the PDF
  // matches the Client Proposal tab, where an empty list shows no table.
  if (siteExclusions.length > 0) {
    const siteRows = siteExclusions.map(x => x.text || '');
  ...
  // NOTES (client-facing, optional) - only printed when the user typed something
  const notesText = (e.proposal_notes || '').trim();
  if (notesText) {
    sectionHeader('NOTES');
    doc.font('Helvetica').fontSize(9).fillColor('#1a1a1a').text(notesText, L, doc.y, { width: W, lineGap: 2 });
    doc.moveDown(0.5);
  }
  ...
  // OPEN RFIs block removed — RFIs are not shown or editable on the Client
  // Proposal tab, so nothing about them belongs in the downloaded PDF. Keep a
  // small gap before the signature block for vertical rhythm.
  doc.moveDown(0.8);
  ...
  // The takeoff is internal and is deliberately NOT in the client proposal.
  //
  // This used to append "PROJECT SCOPE" pages built from the takeoff, listing
  // every shape, plate and misc item, so the PDF handed the GC our member-by-
  // member breakdown. The on-screen proposal never showed it, so the download
  // and the preview disagreed. Removed 2026-08-07 at Steve's request.
  //
  // The client still gets the scope narrative: it is written near the top of
  // this document from proposal_scope, falling back to scope, and is untouched
  // by this. If a member list is ever wanted for a client, it should be an
  // explicit opt-in, not automatic.
  //
  // Note this also governs the copy emailed to the GC on Ready to Submit, since
  // that attachment comes from generateProposalBuffer through this same writer.
