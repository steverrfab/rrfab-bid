...
// Plate length, in inches.
//
// Column E is the authoritative source: the estimator's own weights in column J
// are built from it, and checked across this workbook the ratio between their
// weight and ours comes out at 1.0991 with a spread of zero. The description's
// third dimension does not hold up the same way, so it cannot be used in place
// of column E.
//
// But column E is written in FEET rounded to two decimals, so a clean 20" plate
// arrives as 1.67 ft and lands in the grid as 20.04", which is not a number that
// appears anywhere on the takeoff. Where the description says 20 and column E
// agrees to within what that rounding can explain, the description's whole
// number is used, purely so the grid reads the way the takeoff does.
//
// The rounding is upward, not to nearest: 16" is 1.3333 ft but the workbook
// writes 1.34, and 10" is 0.8333 written as 0.84. So the gap to allow is a whole
// 0.01 ft, which is 0.12". Half of that would leave most rows untouched.
//
// Beyond 0.12" the two are telling us genuinely different things -- this
// workbook has shear plates described as 12", 18" and 21" that all carry 1.25 ft
// in column E -- and column E wins, because that is what their weight was built
// from. Anything inside 0.12" is immaterial either way, so nothing is at risk
// from being generous here.
//
// Bent plates never qualify: their description holds leg widths and the real
// length is only ever in column E.
const PLATE_LENGTH_ROUNDING_IN = 0.125;
function plateLengthIn(dims, pieceLenFt) {
  ...
  const fromColE = Math.round((+pieceLenFt || 0) * 12 * 100) / 100;
  const fromDesc = dims && dims.descLength;
  if (fromDesc > 0 && Math.abs(fromDesc - fromColE) <= PLATE_LENGTH_ROUNDING_IN) return fromDesc;
  return fromColE;
}
