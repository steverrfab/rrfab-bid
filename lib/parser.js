...
// Plate length, in inches. The description wins whenever it gives one.
//
// It is tempting to use column E instead, because the estimator's own weights
// in column J are built from it and it therefore reconciles perfectly. That is
// backwards: it proves column E is what THEY calculated from, not that it is
// the size of the plate. Column E is the length written in feet and rounded up
// to two decimals, so a 10" cap plate arrives as 0.84 ft and comes back out as
// 10.08" -- a number that appears nowhere on the takeoff and describes no plate
// anyone is going to cut.
//
// Worse, column E is sometimes just wrong. This workbook has four shear plates
// described as 9", 12", 15", 18" and 21" that all carry 1.25 ft. Priced off
// column E they became one 15" row of qty 25, silently merging four different
// plates into a size that only one of them is, because the grouping key is
// thickness/width/length. The estimator's total absorbs it. The takeoff does
// not, and neither does the shop.
//
// Measured over the whole workbook, taking the length from the description is
// also CLOSER to the estimator's total, not further: +0.13% against +0.27% for
// column E. The per-row differences are their rounding, and they cancel.
//
// Column E remains the fallback, and is the only source for a continuous run
// ("PL1/2"X8" - Cont. Plate") or a bent plate, where the description carries
// leg widths and the real length is only ever in column E.
function plateLengthIn(dims, pieceLenFt) {
  const fromDesc = dims && dims.descLength;
  if (fromDesc > 0) return fromDesc;
  // Rounded to hundredths of an inch, because 1.34 * 12 is 16.080000000000002
  // in binary floating point and that is not a length anyone wants to look at.
  return Math.round((+pieceLenFt || 0) * 12 * 100) / 100;
}
...
function parsePlateDims(desc, cu) {
  ...
  s = s.replace(/BENT/g, ' ').replace(/CHECKERED/g, ' ')
       ...
       .replace(/["']/g, '');
  // A bent plate's developed width is normally the legs added up, and that
  // reconciles against the estimator's own weight on every bent row in the
  // sample bar one. The exception states its width in words -- "PL3/8"x3"x8" -
  // 6" Wide Bent Plate" -- where the legs sum to 11 but the estimator priced it
  // at 6, matching the words. An explicit width is a deliberate statement and
  // beats adding the dimensions up.
  const widthHint = isBent ? s.match(/(\d+(?:-\d+\/\d+|\/\d+|\.\d+)?)\s*"?\s*WIDE/) : null;
  ...
  const hinted = widthHint ? dimToNumber(widthHint[1]) : NaN;
  const width = isBent
    ? (Number.isFinite(hinted) && hinted > 0 ? hinted : rest.reduce((a, b) => a + b.num, 0))
    : rest[0].num;
  if (!(width > 0)) return null;
  // Store the thickness in the exact spelling the plate weight table uses, so
  // the lookup hits on the first line instead of falling through to parseFloat.
  // PLATE_PSF writes mixed numbers with a space ('1 1/4'), and the grid's
  // dropdown offers the same, so an imported thickness now matches a preset
  // rather than showing up as a one-off option. This matters: parseFloat stops
  // at the slash, so a thickness stored as '1-1/4' would have been read as 1
  // inch thick and a plate priced at a fifth of its real weight.
  const thickness = parsed[0].raw.replace(/^(\d+)-(\d+\/\d+)$/, '$1 $2');
  // The third dimension, when the description carries one. Used only to tidy up
  // the length that comes off column E -- see plateLengthIn.
  const descLength = (!isBent && rest.length >= 2) ? rest[1].num : null;
  return { thickness, width, descLength };
}
    ...
    sheet.eachRow((row, rowNum) => {
      ...
      if (EXTERNAL_SHAPE_MAP[cu]) {
        ...
        if (dims && dims.thickness) {
          const lengthIn = plateLengthIn(dims, pieceLen);
          const qty = pieceCount > 0 ? pieceCount : 1;
          // The part name is part of the key, not just the dimensions. A gusset
          // plate and a stiffener plate can be the same piece of steel and still
          // be two lines on the takeoff, and rolling them into one row makes the
          // grid stop matching the document it came from -- and loses one of the
          // two names, since a group only keeps the first. Identical lines, the
          // same part repeated, still roll up and sum their quantity.
          //
          // Normalised for case and spacing so "PL3/8"X6"x8"" and
          // "PL3/8"x6"x8"" are recognised as the same part rather than split.
          const partName = colD.toUpperCase().replace(/\s+/g, ' ').trim();
          const key = dims.thickness + '|' + dims.width + '|' + lengthIn + '|' + partName;
