...
router.post('/', (req, res) => {
  ...
  db.prepare(
    'INSERT INTO proposal_manual_lines (estimate_id, position, description, amount, hidden, keep_in_total) VALUES (?,?,?,?,0,NULL)'
  ).run(id, position, String(description), +amount || 0);
...
router.put('/:lineId', (req, res) => {
  ...
  const { description, amount, position } = req.body || {};
