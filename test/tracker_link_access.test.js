...
async function run() {
  ...
  r = await call('adm', 'PUT', '/api/users/22', { page_access: ['estimates'] });
  t('an admin cannot edit their own access', r.status === 400 || r.status === 403, r);
  r = await call('adm', 'PUT', '/api/users/21', { page_access: ['estimates'] });
  t('nobody else edits a superadmin', r.status === 400 || r.status === 403, r);
  r = await call('boss', 'PUT', '/api/users/21', { page_access: ['estimates'] });
  t('a superadmin sets their own ribbon', r.status === 200, r);
  r = await call('boss', 'GET', '/api/users');
  const boss21 = r.body.users.find(u => u.id === 21);
  t('superadmin custom list is stored', JSON.stringify(boss21.pages) === '["estimates"]', boss21);
  r = await call('boss', 'PUT', '/api/users/21', { page_access: null });
  t('superadmin back to every page', r.status === 200, r);
